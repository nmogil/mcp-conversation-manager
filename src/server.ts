...
// --- Phase 1, Step 3: Define Data Model ---

interface ConversationState {
  id: string; // Unique identifier for the conversation
  goal: string | null; // The main objective
  summary: string | null; // A running summary
  keyPoints: string[]; // List of important takeaways/decisions
  phase: 'idle' | 'defining_goal' | 'exploring' | 'summarizing'; // Current stage
  // Note: Full transcript handling requires a scalable approach (see Phase 5)
}
...
const conversationAspectTemplate = new ResourceTemplate(
  "conversation://{conversationId}/{aspect}", // URI structure
  {
...
server.resource(
  "conversationAspect", // Internal name for handler registration
  ...
  async (uri, params, extra) => {
    ...
    let determinedConversationId: string;
    ...
    const aspect = params.aspect as string; // Assuming aspect is always a string based on URI pattern
...
server.tool(
  "set_conversation_goal", // Tool name
...
server.tool(
  ...
  "Generates and stores a concise summary of the provided text, typically the recent conversation history.",
  {
...
server.tool(
  "suggest_next_step",
  "Analyzes the current conversation state (goal, phase) and suggests a logical next step or topic to keep the conversation productive and on track.",
  {
    conversationId: z.string().optional().describe("Optional ID for the conversation; uses session ID if omitted.")
  },
...
server.tool(
  "record_key_point",
  "Records a significant takeaway, decision, or action item identified during the conversation.",
  {
